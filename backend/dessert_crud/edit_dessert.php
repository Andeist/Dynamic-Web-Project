...
if (!empty($_FILES['image']['tmp_name'])) {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadFile)) {
        ...
        $query = 'UPDATE desserts SET name = ?, description = ?, image = ?, ingredients = ?, directions = ?, video_embed = ? WHERE id = ?';
    ...
    // Prepare and execute the database query
    $query = 'UPDATE desserts SET name = ?, description = ?, ingredients = ?, directions = ?, video_embed = ? WHERE id = ?';
    $stmt = $conn->prepare($query);
